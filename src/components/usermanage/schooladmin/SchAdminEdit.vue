		...
		methods:{
			...
			reqSchoolData(name, alias, page){
				...
				async function asyncReq(){
					let resp = await Utils.reqSchoolList.call(this, name, alias, page);
	     			...
	     			for(var i in this.school_options) {
	     				if(this.school_options[i].name === this.schname) {
	     					this.school_value = this.school_options[i].id;
	     					break;
	     				}
	     			}
	     		}			
			...
			saveEdit(){
				...
				async function asyncReq(){
					...
					var api = global_.schadmin_update;
					let data = {
						'user_id': this.userid,
